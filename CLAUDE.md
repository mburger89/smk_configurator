...
```bash
swift build --target SMKConfigurator   # build (see note below on why NOT bare `swift build`)
swift run SMKConfigurator               # run the app
swift test                              # run all tests
swift test --filter <SuiteOrTestName>   # run one suite/test
